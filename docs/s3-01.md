...
The XO endpoint itself is hardcoded in `terraform/providers.tf`, not read from
...
backend guide still shows `endpoint` and `force_path_style`, both deprecated in
OpenTofu 1.11.
...
repo and one admin age key survive. That is the whole point of storing it this
way rather than in a password manager.
...
1. Do the Project, group and policy setup from *Who can touch the state*, then
   create the bucket named in `terraform/backend.tf` in `nl-ams` inside that
...
3. Keep a copy of the current state, which is still plaintext at this point:
   ...
   ```sh
   cp terraform/terraform.tfstate terraform/terraform.tfstate.pre-migration
   ```

4. Temporarily let tofu read that plaintext state. In the `encryption` block of
   `terraform/backend.tf`, add the unencrypted method and replace the `state`
